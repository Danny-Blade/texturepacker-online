...
function alignDimension(value, constraint) {
    const safe = Math.max(1, Math.ceil(value));
    if (constraint === 'pot')
        return nextPowerOfTwo(safe);
    if (constraint === 'multiple-of-4')
        return Math.ceil(safe / 4) * 4;
    if (constraint === 'word-aligned')
        return Math.ceil(safe / 2) * 2;
    return safe;
}
function packWithAlgorithm(rects, options, token, reportProgress) {
    const placement = rects.map((r) => ({ id: r.id, width: r.w, height: r.h }));
    const sheets = [];
    const failedIds = [];
...
function handlePack(req) {
    const token = { cancelled: false };
    tokens.set(req.id, token);
    try {
        const algorithms = req.options.packMode === 'fast'
            ? ['shelf']
        ...
        for (let i = 0; i < algorithms.length; i++) {
            ...
            const attempt = packWithAlgorithm(req.rects, { ...req.options, algorithm: alg }, token, () => {
                const base = i / total;
